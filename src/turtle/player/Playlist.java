...
public class Playlist
{
	...
	private void CheckDir(File rootNode, int depth)
	{
		...
		try
		{
			...
			for (String mp3 : rootNode.list(isMP3))
			{
				...
				artist = metaDataReader
						.extractMetadata(MediaMetadataRetriever.METADATA_KEY_ARTIST);
				album = metaDataReader
						.extractMetadata(MediaMetadataRetriever.METADATA_KEY_ALBUM);
				length = Double
						.parseDouble(metaDataReader
			...
			{
				if (depth < MAX_DIR_SCAN_DEPTH) // Avoid Stack Overflow -
												// symbolic link points to a
												// parent dir
				{
	...
	public Track PreviousTrack()
	{
		...
		{
			historyPosition++;
			listPosition = historyList.get((historyList.size() - 1)
					- historyPosition);
	...
	public String[] GetTracksByAlbum(int position)
	{
		...
		{
			String v = String.valueOf(newList.get(k).GetNumber()) + ". "
					+ newList.get(k).GetTitle();
	...
	public static abstract class PlaylistObserverAdapter implements
			PlaylistObserver
	{
		@Override
